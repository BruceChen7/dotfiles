...
# z.lua init is relatively expensive; cache the generated shell snippet.
_zlua_script="$HOME/.local/bin/z.lua/z.lua"
_zlua_cache="${XDG_CACHE_HOME:-$HOME/.cache}/zsh/zlua-init.zsh"
if [[ -r "$_zlua_script" ]]; then
    if [[ ! -s "$_zlua_cache" || "$_zlua_script" -nt "$_zlua_cache" ]]; then
        mkdir -p "${_zlua_cache:h}"
        lua "$_zlua_script" --init zsh enhanced once echo >| "$_zlua_cache"
    fi
    source "$_zlua_cache"
fi
unset _zlua_script _zlua_cache
...
# bun completions
[ -s "/Users/ming.chen/.bun/_bun" ] && source "/Users/ming.chen/.bun/_bun"
# wt init is generated by the binary; cache it to avoid a process on every shell startup.
_wt_bin="${commands[wt]:-}"
_wt_cache="${XDG_CACHE_HOME:-$HOME/.cache}/zsh/wt-init.zsh"
if [[ -n "$_wt_bin" ]]; then
    if [[ ! -s "$_wt_cache" || "$_wt_bin" -nt "$_wt_cache" ]]; then
        mkdir -p "${_wt_cache:h}"
        command wt config shell init zsh >| "$_wt_cache"
    fi
    source "$_wt_cache"
fi
unset _wt_bin _wt_cache
